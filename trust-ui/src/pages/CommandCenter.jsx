...
export function CommandCenter() {
  const { user } = useAuth();
  const [trust, setTrust] = useState({ data: null, loading: true, error: "" });
  const [fraud, setFraud] = useState({ data: null, loading: true, error: "" });
  const [advisor, setAdvisor] = useState({ data: null, loading: true, error: "" });
  const [twin, setTwin] = useState({ data: null, loading: true, error: "" });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setTrust({ data, loading: false, error: "" });
      } catch (err) {
        if (!cancelled) setTrust({ data: null, loading: false, error: err.message });
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setFraud({ data, loading: false, error: "" });
      } catch (err) {
        if (!cancelled) setFraud({ data: null, loading: false, error: err.message });
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setAdvisor({ data, loading: false, error: "" });
      } catch (err) {
        if (!cancelled) setAdvisor({ data: null, loading: false, error: err.message });
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setTwin({ data, loading: false, error: "" });
      } catch (err) {
        if (!cancelled) setTwin({ data: null, loading: false, error: err.message });
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setMonitor({ metrics, system, services, loading: false, error: "" });
        }
      } catch (err) {
        if (!cancelled) {
          setMonitor({
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-10 pb-4">
      ...
      <div className="space-y-10">
        ...
        >
          <TrustWidget data={trust.data} loading={trust.loading} error={trust.error} />
          <AdvisorWidget data={advisor.data} loading={advisor.loading} error={advisor.error} />
        </DashboardSection>
        ...
        >
          <FraudWidget data={fraud.data} loading={fraud.loading} error={fraud.error} />
        ...
        >
          <LoanWidget />
          <DigitalTwinWidget data={twin.data} loading={twin.loading} error={twin.error} />
          <div className="sm:col-span-2 xl:col-span-1">
            <ForecastWidget data={twin.data} loading={twin.loading} error={twin.error} />
          </div>
        </DashboardSection>
      </div>
    </div>
